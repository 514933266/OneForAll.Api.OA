...
namespace OA.Host.Controllers
{
    ...
    public class OAJobDutysController : BaseController
    {
        private readonly IOAJobDutyService _service;
        public OAJobDutysController(IOAJobDutyService service)
        {
            _service = service;
        }
        ...
        /// <returns>列表</returns>
        [HttpGet]
        public async Task<IEnumerable<OAJobDutyDto>> GetListAsync([FromQuery] string key = default)
        {
        ...
        public async Task<BaseMessage> AddAsync([FromBody] OAJobDutyForm form)
        {
        ...
        [HttpPut]
        [CheckPermission(Action = ConstPermission.EnterView)]
        public async Task<BaseMessage> UpdateAsync([FromBody] OAJobDutyForm form)
        {
            var msg = new BaseMessage();
